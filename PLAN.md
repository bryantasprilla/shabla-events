...
- **Milestone 11 result**: a real `workflow_dispatch` run completed successfully in 5m8s (run `36181794645`), landing commit `9b33410` from the `shabla-events-bot` identity. Confirmed for real on the actual `ubuntu-latest` runner (not just locally on Windows): `llama-cpp-python` installs and loads the model successfully via plain `pip install -r requirements.txt` (no special wheel-index flag needed on Linux, unlike Windows), the model-weights cache saves/restores correctly (~4.59GB), and change-detection correctly produced zero re-processed articles against the already-committed DB (so no LLM calls were needed this run -- a good idempotency check, though it means extraction-under-CI specifically hasn't been observed yet; that'll happen naturally once real new articles show up). New finding from running under GitHub's IP ranges specifically: `dobrichonline.com` returned `403 Forbidden` (it fetched fine from the Windows dev machine) -- likely blocking known cloud/datacenter IP ranges. Left as-is for the error-isolation/issue-auto-filing system to track rather than fixed immediately.
